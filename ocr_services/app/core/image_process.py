...
def detect_and_correct_skew(pil_image):  
    ...
    print(f"   [detect_and_correct_skew] Input: {width}x{height}, aspect={width/height:.2f}")
    ...
    try:  
        ...
        confidence = results.get("orientation_conf", 0)
        
        print(f"   [detect_and_correct_skew] OSD: rotate={rotation_angle}°, orient={orientation}, conf={confidence}")
          
        if rotation_angle != 0:  
            # Xoay ảnh về hướng đã sửa  
            rotated = imutils.rotate_bound(image_array, angle=rotation_angle)  
            result = Image.fromarray(rotated)
            print(f"   [detect_and_correct_skew] ROTATED {rotation_angle}° -> {result.size}")
            return result  
        else:  
            print(f"   [detect_and_correct_skew] No rotation needed")
            return pil_image  
              
    except Exception as e:      
        print(f"   [detect_and_correct_skew] ERROR: {e}")  
        return pil_image
