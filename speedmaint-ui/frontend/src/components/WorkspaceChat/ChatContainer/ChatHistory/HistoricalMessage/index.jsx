...
function ImageGallery({ images = [] }) {
  ...
  return (
    <div className="mt-3">
      <p className="text-xs text-white/40 mb-2 uppercase tracking-wider">Hình ảnh liên quan</p>
      <div className="flex flex-wrap gap-2">
        {images.map((objKey, idx) => {
          const src = `${IMAGE_BASE}/${objKey}`;
          return (
            <img
              key={idx}
              src={src}
              alt={`Hình ảnh ${idx + 1}`}
              className="h-28 w-auto rounded-lg border border-white/10 object-cover cursor-zoom-in hover:opacity-90 transition-opacity"
