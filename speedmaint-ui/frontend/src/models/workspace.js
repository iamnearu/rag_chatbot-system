...
import { baseHeaders, baseJsonHeaders, safeJsonParse } from "@/utils/request";
...
const Workspace = {
  ...
  modifyEmbeddings: async function (slug, changes = {}) {
    try {
      ...
      if (!response.ok) {
        const errorMessage = data.detail || data.error || `Error: ${response.status}`;
        return { workspace: null, message: errorMessage };
      }

      // Check for success: false in response body (legacy support)
